...
1. 结构是否标准：每个 Agent Skill 必须位于 `skills/<skill-name>/SKILL.md`。
...
5. 内容是否完整：至少包含适用场景、工作流程或使用步骤、Prompt 示例。
6. 步骤是否可复现：不要只写概念，要能照着执行。
7. Prompt 是否可直接复制：占位符、变量和输入输出要求要明确。
8. 是否存在重复：搜索 `skills/` 和 `index/skills.json`，避免同类内容重复沉淀。
9. 是否适合公开：确认没有隐私信息、内部资料、未授权搬运内容。
